...
int main(int argc, char ** argv) {
    if (argc != 9) {
        std::cerr << "usage: " << argv[0]
                  << " KERNEL_DIR RUNTIME_SO K N BLOCK_N THREADS ITERS BATCHES\n";
        return 2;
    }
    ...
    triton_w8_kernel * kernel =
        triton_w8_kernel_create(kernel_dir, k, n, block_n);
    ...
    enum Mode { triton_mode, runtime_mode };
    std::atomic<Mode> mode{triton_mode};
    ...
    for (int tid = 0; tid < threads; ++tid) {
        workers.emplace_back([&, tid] {
            while (true) {
                ...
                if (mode.load(std::memory_order_relaxed) == runtime_mode) {
                    if (tid == 0) {
                    ...
                    while ((block = next_block.fetch_add(
                        ...
                        gemv(xq.data(), x_scale, packed.data(), scale.data(),
                             runtime_output.data(), k, n, n / 4, block_n / 4,
                             block, 1);
                    }
                } else {
                    int64_t block;
    ...
    launch(triton_mode);
    launch(runtime_mode);
    if (triton_output != runtime_output) {
        throw std::runtime_error("threadpool output mismatch");
    }
    const double triton_us =
        median_us([&] { launch(triton_mode); }, 50, iterations, batches);
    ...
    std::cout << "PASS K=" << k << " N=" << n
              ...
              << "triton_threadpool_us=" << triton_us << '\n'
