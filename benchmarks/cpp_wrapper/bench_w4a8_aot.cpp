...
namespace {
using AotKernel = void (*)(void *, void *, void *, void *, void *,
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        const int batches = argc == 7 ? std::stoi(argv[6]) : 7;
        if (k <= 0 || n <= 0 || k % 32 != 0 || n % 32 != 0) {
            throw std::runtime_error("K and N must be positive multiples of 32");
        }
        ...
        std::vector<uint8_t> packed(
            static_cast<size_t>(tiles) * groups * 16 * 32);
        std::vector<float> weight_scale(
            static_cast<size_t>(tiles) * groups * 32);
        ...
        AotKernel kernel =
            kernel_library.symbol<AotKernel>("_w4a8_grouped_gemv_kernel");
        CKernel c_kernel =
            c_library.symbol<CKernel>("w4a8_microtile_c");

        auto run_triton = [&] {
            kernel(x.data(), x_scale.data(), packed.data(),
                   weight_scale.data(), triton_output.data(),
                   0, 0, 0, 1, 1, 1);
        };
        auto run_c = [&] {
            c_kernel(x.data(), x_scale.data(), packed.data(),
        ...
        for (int i = 0; i < n; ++i) {
            max_abs =
                std::max(max_abs, std::abs(triton_output[i] - c_output[i]));
        }
        if (max_abs > 1.2e-5f) {
            throw std::runtime_error(
                "AOT output mismatch, max_abs=" + std::to_string(max_abs));
        }

        const double triton_us =
            median_us(run_triton, 20, iterations, batches);
        const double c_us =
            median_us(run_c, 20, iterations, batches);
        std::cout << "PASS K=" << k << " N=" << n << '\n'
                  << "direct_aot_triton_us=" << triton_us << '\n'
                  << "c_microtile_us=" << c_us << '\n'
