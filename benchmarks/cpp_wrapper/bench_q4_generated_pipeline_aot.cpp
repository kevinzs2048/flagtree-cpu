...
using MatrixKernel = void (*)(void *, void *, void *, void *, void *, int32_t,
...
std::pair<double, double> pairedMedianUs(First &&first, Second &&second,
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const bool validM =
        m > 0 && ((m <= 16 && m % 4 == 0) || (m > 16 && m % 16 == 0));
    if (!validM || n <= 0 || n % 4 || k <= 0 || k % 32 || iterations <= 0 ||
        batches <= 0 || warmup < 0)
      throw std::runtime_error(
          "requires M=4/8/12 or M%16=0, N%4=0, K%32=0 and positive counts");

    SharedObject packLibrary(argv[1]);
    SharedObject matrixLibrary(argv[2]);
    PackKernel pack = packLibrary.symbol<PackKernel>(
        "_pack_lhs_qsi8d32p_panel4_scalar_kernel");
    MatrixKernel matrix =
        matrixLibrary.symbol<MatrixKernel>("_q4_prefill_i8mm_kai_kernel");
    std::unique_ptr<SharedObject> referenceMatrixLibrary;
    MatrixKernel referenceMatrix = nullptr;
    if (argc >= 10) {
      referenceMatrixLibrary = std::make_unique<SharedObject>(argv[9]);
      referenceMatrix = referenceMatrixLibrary->symbol<MatrixKernel>(
          "_q4_prefill_i8mm_kai_kernel");
    }

    const int groups = k / 32;
    const int panels = m / 4;
    const int matrixBlocks = m <= 16 ? 1 : m / 16;
    const int outputTiles = n / 4;
    std::vector<uint16_t> input(static_cast<size_t>(m) * k);
    std::vector<uint8_t> lhsPacked(static_cast<size_t>(panels) * groups * 136);
    std::vector<uint8_t> rhsPacked(static_cast<size_t>(outputTiles) * groups *
                                   72);
    std::vector<uint16_t> output(static_cast<size_t>(m) * n);
    std::vector<uint16_t> referenceOutput(static_cast<size_t>(m) * n);
    ...
    auto runPack = [&] {
      for (int pid = 0; pid < panels; ++pid)
        pack(input.data(), lhsPacked.data(), lhsPacked.data(), m, k, pid, 0, 0,
             panels, 1, 1);
    };
    auto runMatrix = [&] {
      for (int pidM = 0; pidM < matrixBlocks; ++pidM) {
        for (int pidN = 0; pidN < outputTiles; ++pidN) {
          matrix(lhsPacked.data(), lhsPacked.data(), rhsPacked.data(),
                 rhsPacked.data(), output.data(), pidM, pidN, 0, matrixBlocks,
                 outputTiles, 1);
        }
      }
    };
    auto runReferenceMatrix = [&] {
      for (int pidM = 0; pidM < matrixBlocks; ++pidM) {
        for (int pidN = 0; pidN < outputTiles; ++pidN) {
          referenceMatrix(lhsPacked.data(), lhsPacked.data(), rhsPacked.data(),
                          rhsPacked.data(), referenceOutput.data(), pidM, pidN,
                          0, matrixBlocks, outputTiles, 1);
        }
      }
    };
    auto runPipeline = [&] {
    ...
    runPipeline();
    if (output != expected)
      throw std::runtime_error("generated pipeline is not deterministic");
    if (referenceMatrix) {
      runReferenceMatrix();
      if (referenceOutput != output)
        throw std::runtime_error("candidate/reference matrix output mismatch");
    }
    ...
    std::cout << "PASS generated Q4 pipeline M=" << m << " N=" << n
              << " K=" << k << '\n'
    ...
    if (referenceMatrix) {
      auto [candidateUs, referenceUs] = pairedMedianUs(
          runMatrix, runReferenceMatrix, warmup, iterations, batches);
      std::cout << "candidate_matrix_paired_us=" << candidateUs << '\n'
                << "reference_matrix_paired_us=" << referenceUs << '\n'
                << "candidate_speedup=" << referenceUs / candidateUs << "x\n"
                << "candidate_reference_bit_exact=true\n";
    }
