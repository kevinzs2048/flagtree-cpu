...
using CKernel = void (*)(const int8_t *, const float *, const float *,
                         const uint8_t *, const float *, const float *, float *,
                         int, int);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    for (int group = 0; group < groups; ++group) {
      ...
      x_sum[group] = static_cast<float>(sum) * x_scale[group];
    }
    ...
    for (size_t i = 0; i < weight_scale.size(); ++i) {
      ...
      const float minimum = -0.04f + (i % 23) * 0.003f;
      packed_offset[i] = minimum + 8.0f * weight_scale[i];
    }
    ...
    CKernel c_kernel =
        c_library.symbol<CKernel>("w4a8_q41_microtile4_c");

    auto run_triton = [&] {
      kernel(x.data(), x_scale.data(), x_sum.data(), packed.data(),
             weight_scale.data(), packed_offset.data(), triton_output.data(),
             0, tiles, 0, 0, 0, 1, 1, 1);
    };
    auto run_c = [&] {
      c_kernel(x.data(), x_scale.data(), x_sum.data(), packed.data(),
               weight_scale.data(), packed_offset.data(), c_output.data(), k,
               n);
    };
